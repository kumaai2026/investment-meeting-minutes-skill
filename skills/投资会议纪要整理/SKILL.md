...
- First use plain SenseVoice through the local FunASR runtime as the primary transcript and timestamp source. Do not force `spk_model=cam++` or `fsmn-vad` into the default pre-agent transcription path, because the ambiguity timestamp workflow is more stable when it uses direct SenseVoice timestamps. Use cam++ speaker diarization and VAD only as an explicit speaker-separation enhancement, not as the mandatory source for doubtful-term timestamps.
- Keep Fun-ASR-Nano-2512 as the auxiliary proofreading transcript for cross-checking finance terms, company names, stock codes, numbers, and English abbreviations. Do not replace the primary SenseVoice transcript automatically only because Nano differs; surface conflicts for human review.
- Use `scripts/transcribe_audio.py` as the standard entry point. Its default `--engine auto` tries plain SenseVoice first and falls back to Whisper only when SenseVoice/FunASR is unavailable. Use `--engine fun-asr-nano --nano-hub ms --nano-model FunAudioLLM/Fun-ASR-Nano-2512` for the auxiliary model or comparison runs. Use `--speaker-diarization` only when speaker labels are required, `--sensevoice-vad` only when VAD chunking is explicitly needed, and `--require-speaker-diarization` only when a speaker-labeled run must fail instead of falling back to plain transcription.
...
- Every row in `## 二、存疑与待确认` must include a `时间戳` column before `原始表述` and a final blank `人工确认` column for the user's manual proofreading. Use the timestamp from the transcript/audio segment when available, preferably in `HH:MM:SS` or `MM:SS` form. If audio is available but the ASR model cannot emit reliable sentence/word timestamps, fall back to a VAD/audio-segment anchor for the relevant utterance; do not leave audio-derived doubtful items as `未提供`. Do not estimate timestamps from the relative position of the cleaned meeting note or summary text, because edited prose length and speaking duration drift too much. Only use `未提供` when the source is text-only or the audio segment genuinely cannot be located. Leave the `人工确认` cells empty.
- Every uncertain fragment that appears in the original-text body must also carry its ambiguity timestamp inline, immediately after the bold doubtful fragment, using `**存疑词**（存疑时间戳：HH:MM:SS）` or, only for text-only/unlocatable sources, `**存疑词**（存疑时间戳：未提供）`. This applies to doubtful fragments in bracket headings and正文 paragraphs. Do not rely on the final ambiguity table alone.
...
- The ambiguity table must use the columns `时间戳 | 原始表述 | 当前判断 | 存疑原因 | 候选项 | 人工确认` in that order. Leave `人工确认` cells empty for the user.
...
- For uncertain words in the original-text section, keep the source wording and mark the doubtful fragment in bold markdown. Immediately after each bold doubtful fragment, include the inline ambiguity timestamp in the form `（存疑时间戳：HH:MM:SS）`. When audio exists, use ASR sentence/word timestamps first and VAD/audio-segment anchors as the fallback; never derive timestamps by proportional alignment against the cleaned note text. Use `（存疑时间戳：未提供）` only for text-only or genuinely unlocatable sources. In Word export, render these doubtful fragments as bold + underline so they are visibly marked.
