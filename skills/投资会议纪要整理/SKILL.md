...
- First use SenseVoice through the local FunASR runtime as the primary transcript. By default, `scripts/transcribe_audio.py` now tries SenseVoice speaker diarization with `spk_model=cam++`, then writes readable speaker-labeled TXT plus sentence-level JSON when `--output-format json|all` is used. The wrapper resolves downloaded local model directories before remote model names, so normal runs do not depend on online model lookup after the models have been cached. If the local FunASR runtime does not support speaker diarization yet, it degrades to plain SenseVoice transcription unless `--require-speaker-diarization` is passed.
...
- Use `scripts/transcribe_audio.py` as the standard entry point. Its default `--engine auto` tries SenseVoice first and falls back to Whisper only when SenseVoice/FunASR is unavailable. Use `--engine fun-asr-nano --nano-hub ms --nano-model FunAudioLLM/Fun-ASR-Nano-2512` for the auxiliary model or comparison runs. Use `--no-speaker-diarization` to disable speaker labels, or `--require-speaker-diarization` when a run must fail instead of falling back to plain transcription.
